...
class M_salesInvoice
{
	...
	// for barcode
	function checkBarcodeAvailable($barcode){
		global $con;
		 $query="SELECT itemMasterId  FROM itemMaster where itemCode='".$barcode."' and status=1";
		$result = mysqli_query($con, $query);
		return mysqli_num_rows($result);
	}

	function getMaterialsForBarcode($materialsName,$regularCustomerId,$privilageId,$branchId)
	{
		global $con; 
		if($privilageId==1 || $branchId==2 || $branchId==3){
			 $query = "SELECT S.itemMasterId,S.stockId,S.expiryDate,IM.importLocalStatus,IM.itemName,IM.itemCode,IM.packing,IM.maxretailPrice,vat,stock_".$privilageId."_".$branchId." as stockValue,minimumRate		       
	...
	function getCustomerName($regularCustomerId){
		global $con;
		$query="SELECT customerNo,customerName,vatNumber,contactNo_1  FROM regularCustomer where regularCustomerId='".$regularCustomerId."'";
		$result = mysqli_query($con, $query);
		return $result;

...
$maxCustomerNo         = $objCSalesInvoice->getMaxCustomerNo();
...
if(isset($_POST['submit']))
{
   ...
   $zakatInvoiceType='0200000';
	...
	$objPath->setHeader('counterSalesInvoice','Success!!!','counterSalesInvoice');
	}else{
   ...
   $zakatInvoiceType='0200000';
	...
	if($invoiceId>0)
	{
		for($i=0;$i<count($itemMasterId);$i++)
		{
			...
			$itemUnitId				=	$itemUnitRowArray [0];
			$unitFraction			=	$itemUnitRowArray [1];
			$float1 = (float)$purchasePriceRow[$i];
            $float2 = (float)$netWeightRow[$i];

			 if (is_numeric($float1) && is_numeric($float2)) 
               $purchasePrice = $float1 * $float2;
		    else
			     $purchasePrice          =0;
			//$purchasePrice          =   $purchasePriceRow[$i]*$netWeightRow[$i];//$unitFraction*$quantityRow[$i];
			$totalCostValue         =   $totalCostValue+$purchasePrice;
	...
	/*-------------------------------insert To  invoiceDetails Table Ends------------------------------------*/
		...
		//header("location:welcome.php?page=countersalesInvoicePrint&invoiceId=$invoiceId&referanceNo=1");
	if($printType==1)
	{
		header("location:welcome.php?page=countersalesInvoicePrint&invoiceId=$invoiceId&referanceNo=1");
	}
	else
	{
		...
		header("location:welcome.php?page=counterSalesInvoiceThermalPrint&invoiceId=$invoiceId&referanceNo=1");
		//header("Location:".$_SERVER['HTTP_HOST'].'/naf/modules/counterSalesInvoice/admin/includes/salesInvoiceThermalPrint.php?invoiceId='. urlencode($invoiceId) . "&referanceNo=1");
	}
			...
			<div class="col-sm-12 col-md-12 col-lg-12">
					...
					<div class="row" >
							...
							<div class="form-group col-sm-2 col-md-2 col-lg-2" style=" <?php echo $showCustomerCode; ?>" >
								...
								<br>
									<button type="button" class = "btn btn-info btn-sm" style="margin-left: -21px;margin-top: 6px;" data-toggle = "modal" data-target = "#myModalCustomer"  title="view" onclick="">
		...
		<table class="table table-bordered example "id="salesItems" style="font-size: 11px !important;">
			...
			<tr>
				...
				<tr>
					...
					<input type="text"  style="width:10%;padding: 0%;border: 0px !important;" class="input-sm" value="%" readonly/>
				...
				<input type="hidden" name="damagedGoodsAmount" id="damagedGoodsAmount" class="input-sm" value="0" onkeyup="checkNumber(this.id);getDiscountAmount();clearRound();calculateSum();" onchange="checkNumber(this.id);"    style="text-align: right;width: 100%"  style="text-align: right;" />
				
				
				
				<input type="hidden" name="cuttingCharge" id="cuttingCharge" class="input-sm" onkeyup="checkNumber(this.id);clearRound();calculateSum();" onchange="checkNumber(this.id);"  value="0"   style="text-align: right;width: 100%"  style="text-align: right;" />
				
				
				</tr>
				<tr>
					<th  style="text-align: right;"  colspan="8"><span class='footerOfTable'>VAT AMOUNT</span></th>
					<td colspan='2' >
				...
				<tr>
					<th  style="text-align: right;"  colspan="8"><span class='footerOfTable'>NET AMOUNT</span></th>
...
<!------------------------------- Add Customer Start---------------------->
<form name="addNewCustomerForm" id="addNewCustomerForm" method= "post" action = "" autocomplete= "off"> 
      					...
      					<div class="col-sm-12 col-md-12 col-lg-12">
							  ...
							  <div class="col-sm-4 col-md-4 col-lg-4">
										<div class="form-group">
											...
											<input name="customerName" autocomplete="off" type="text" required id="customerName1" class="form-control input-sm" />
										</div>
									   <div class="form-group">
										...
										<div class="form-group">
										 ...
										 <div class="form-group">
											...
											<input type="text" name="vatNumber" class="form-control input-sm" onKeyUp="checkNumeric(this.id)"  id="vatNumber1" /> 
										...
										<?php 
											...
											if($_COOKIE['mainBranch']=='D'){
												$option = '<option value="D">DAMMAM BRANCH</option>';
											}
											  else if($_COOKIE['mainBranch']=='R'){
												$option = '<option value="R">RIYAD BRANCH</option>';
											}
											  else if($_COOKIE['mainBranch']=='M'){
												$option = '<option value="M">MAKKAH BRANCH</option>';

											  }
											  elseif($_COOKIE['mainBranch']=='J'){
												$option = '<option value="J" selected>JEDDAH BRANCH</option>';
											  }*/
            					...
            					<div class="col-sm-4 col-md-4 col-lg-4">
									<div class="form-group">
										...
										<input name="customerCode" autocomplete="off" type="text" required readonly id="customerCode" value="<?php echo $maxCustomerNo;?>"  class="form-control input-sm" />
           							</div>
            						<div class="form-group">
           								<p>
            				...
            				<div class="row">
            					<center>
									<button type = "button"  class = "btn btn-success btn-sm" name="addNewCustomer" id="addNewCustomer">
...
function getMaterialRow(itemMasterId,itemName,itemCode,vat,sellingPrice,purchasePrice,stockId,stockValue,minimumRate)
{
	...
	var invTypes=$('input[name="invType"]:checked').val();
	$.ajax({
                type: "POST",
                url: "../../../../modules/salesInvoice/admin/ajax/getMaterialUnitSalesInvoice.php",
				data: {itemMasterId:itemMasterId,invType:invTypes},
                success: function(data)
                {
...
$(function(){
	...
	$("#materialSearch").autocomplete({
	   source: function(request, response) {
		 $.getJSON("../../../../modules/salesInvoice/admin/ajax/searchMaterials.php", {
			 term  : $('#materialSearch').val(),regularCustomerId:$('#regularCustomerId').val(),flag:$('#flag').val()}, 
				  response);
	  },
		  minLength: 0,
		  focus: function( event, ui ) {
...
// 	$('#materialSearch').on('input', function(e) {
// 	var barcode = $('#materialSearch').val()
// 	// var barcode = $('#materialSearch').val().trim(); // Trim to remove leading/trailing spaces
// 	$.ajax({
// 	type: "POST",
// 	cache:false,
...
$("#addNewCustomer").click(function() {
	...
	var contactNo_1 	=	$('#contactNo_1').val();
	var branchId 		=	$('#branchId').val();	

	if(customerName!='' && contactNo_1!='' && branchId!=''){
	
 var r = confirm("Are You Sure To Add New Customer ");
   if (r == true) {
    var url = "../../../../modules/counterSalesInvoice/admin/ajax/addNewCustomerByAjax.php"; 
	$.ajax({
           ...
           success: function(data)
           {
			// console.log(data.test);
			   if(data.result=="1"){
				    alert("your Customer Added SuccessFully");
					$("#customerName").val(data.customerName);
					$("#regularCustomerId").val(data.regularCustomerId);
					$("#customerCode").val(data.customerNo);
					$("#vatNumber").val(data.vatNumber);
					$("#customerPhone").val(data.contactNo_1);				
					
					//$(':input[id="advanceAmount"]').prop('disabled', true);
			   }
			   else{
					alert("Customer No Duplication");
				    
				}
			$("#addNewCustomerForm").trigger('reset');
